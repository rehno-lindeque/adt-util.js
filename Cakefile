...
# Maybe (() -> IO) -> () -> IO
build = (callback) -> (concatSrcFiles libFiles) (writeFile 'build/adt-util.js') callback

# Maybe (() -> IO) -> () -> IO
minify = (callback) -> ->
  path.exists 'node_modules/.bin/uglifyjs', (exists) ->
    tool = if exists then 'node_modules/.bin/uglifyjs' else 'uglifyjs'
    path.exists 'build/adt-util.js', (exists) ->
      if exists
        exec "#{tool} --no-copyright build/adt-util.js > build/adt-util.min.js", (err, stdout, stderr) ->
          throw err if err
...
# Maybe (() -> IO) -> () -> IO
wrap = (callback) ->
    filename = 'adt-util.js'
    filenameMin = 'adt-util.min.js'
    ...
    # Maybe (() -> IO) -> () -> IO
    wrap = (callback) -> write (logDone 'adt-util.js') callback
    wrapMin = (callback) -> writeMin (logDone 'adt-util.min.js') callback
...
task 'clean', "Cleanup all build files and distribution files", ->
  exec "rm -rf build;rm dist/adt-util.js;rm dist/adt-util.min.js;rm dist/adt-util.module.js;rm dist/adt-util.module.min.js", (err, stdout, stderr) ->
    console.log stdout + stderr
